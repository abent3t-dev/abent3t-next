...
export default function MaximoContractsTab() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      api.get<PaginatedResponse<MaximoContract>>(
        `/maximo/contracts?${queryParams.toString()}`,
      ),
  });

  const contracts = data?.data ?? [];
  const meta = data?.meta;
  const hasFilters = !!search || !!statusFilter || !!hasContractFilter;
      ...
      <div className="bg-white p-4 rounded-lg shadow">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select
            ...
            onChange={(e) => {
              setHasContractFilter(e.target.value);
              setPage(1);
            }}
            className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#52AF32] text-gray-900 bg-white"
          >
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-center">
                          <span className={`inline-flex px-2.5 py-1 text-xs font-medium rounded-full ${maximoStatusBadgeClass(contract.status)}`}>
                            {dash(contract.status)}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3 text-center text-sm text-gray-600">
                          ...
                          {contract.purchview_count > 1 && (
                            <span className="ml-1 text-xs text-gray-400">
                              ({contract.purchview_count} PURCHVIEW)
                            </span>
                          )}
                        </td>
                      </tr>
